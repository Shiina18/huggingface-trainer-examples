...
from transformers.trainer_pt_utils import (
    find_batch_size, nested_concat, nested_numpify,
    IterableDatasetShard, nested_truncate,
)
from transformers.trainer_utils import (
    EvalLoopOutput, has_length, EvalPrediction,
...
class GlobalPointerEval(transformers.Trainer):
    """Copied from source code, and modified label collection process noted with
    `# modified`. The reason is that `np.concatenate` used under the hood is
    extremely slow for large arrays, and causes the evaluation seeming to be stuck.
    """
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if args.eval_accumulation_steps is not None and (
                ...
                if preds_host is not None:
                    ...
                    # )
                    # modified
                    preds = [set(zip(*np.where(y > 0))) for y in logits]
                ...
                if labels_host is not None:
                    ...
                    # )
                    # modified
                    labels = [set(zip(*np.where(y > 0))) for y in labels]
